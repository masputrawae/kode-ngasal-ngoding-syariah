...
def convert_wikilinks_to_md(text):
    pattern = r'(!?)\[\[([^\[\]]+?)(?:\|([^\[\]]+))?\]\]'
    # # Ngak Tahu ini kepakai atau tidak nanti, penting aku simpan dulu daripada hilang
    # optional_pattern = r'\[\[([^|\]#]+)(?:#[^\]]*)?(?:\|([^\]]+))?\]\]' 
    ...
    def replacer(match):
        ...
        if is_embed:
            ...
            return f'[{alias}]({title})'  # Teks link
...
if __name__ == "__main__":
    ...
    source_files = [f for f in glob.glob("obsidian_vault/**/*", recursive=True) if os.path.isfile(f)]
    target_files = [os.path.join("content", os.path.relpath(f, "obsidian_vault")) for f in source_files]
